...
# Reference existing volumes from training
...
def _get_policy():
    """Lazy initialization of policy on first request."""
    global _policy, _modality_config
    if _policy is None:
        import shutil

        # More aggressive cache clearing - clear BEFORE any imports that might trigger Eagle processor loading
        cache_paths = [
            Path.home() / ".cache" / "huggingface" / "modules" / "transformers_modules" / "eagle2_hg_model",
            Path.home() / ".cache" / "huggingface" / "hub" / "models--Eagle-X6--Eagle2.5-VL-7B",
        ]
        ...
        # NOW import gr00t modules (transformers 4.57.1 should have VideoInput support)
        import transformers
        ...
        print("Initializing GR00T policy...")
...
def serve(body: dict):
    ...
    if _modality_config is not None:
        ...
        if missing:
            return {"error": f"Missing required observation keys: {missing}", "expected": list(expected_keys)}, 400
